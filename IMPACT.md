...
| 발견 | repo | 유형 | PR | 상태 | 점수 |
|------|------|------|----|------|------|
| `deleteBulkMetadata` N+1 | [immich](https://github.com/immich-app/immich) · 108k★ | N+1 (루프 안 순차 DELETE, item당 왕복 1회) | [#30163](https://github.com/immich-app/immich/pull/30163) | 🟡 draft | — |

## 규칙

- cleanscore가 찾은 이슈로 낸 PR만 기록한다.
- **손 검증 필수** — 정적 분석은 후보만 뽑는다. 오탐 PR은 툴 신뢰를 깎으므로 금지.
  (예: immich에서 `execute` 4곳이 잡혔지만, 3곳은 락 재시도 루프 = 오탐. 사람이 걸러 1곳만 PR.)
- **머지되면 +1.** 닫히면 0. 정직하게.
